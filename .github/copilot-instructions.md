...
- `src/crucible/` - Main package directory
...
- `src/train.py` - Training entry script (delegates to `crucible.train.tuner`)
- `src/webui.py` - Web UI entry script (delegates to `crucible.webui.interface`)
- `src/api.py` - API server entry script (delegates to `crucible.api.app`)
- `tests/` - Test suite
...
- `src/crucible/v1/` - Version 1 package directory
...
- Hyperparameters are defined using dataclasses in `src/crucible/hparams/`

### Model Support

- New model support is added through model patches in `src/crucible/model/`
- Visual models use the visual utilities in `src/crucible/model/model_utils/visual.py`
- Quantization support is in `src/crucible/model/model_utils/quantization.py`

### Data Processing

- Dataset definitions are in `data/dataset_info.json`
- Data templates and processors are in `src/crucible/data/`

### Training

- Training pipelines are in `src/crucible/train/`
...
- **CLI Training**: `crucible-cli train --config examples/train_lora/llama3_lora_sft.yaml`
- **Web UI**: `crucible-cli webui` or `python src/webui.py`
- **API Server**: `crucible-cli api` or `python src/api.py`
- **Chat Interface**: `crucible-cli chat --model_name_or_path MODEL_PATH`
